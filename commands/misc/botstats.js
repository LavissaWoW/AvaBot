...
module.exports.run = async(bot, message, args, langue) => {
    ...
    if(!command){
        ...
        embed.addField("🇨 Commands", `I have **${bot.commands.size} commands**!`, false)
        embed.addField("🇦 Aliases", `I have **${bot.commands.map(e => e.help.alias.length).reduce((x,y) => x + y)} aliases**!`, false)
