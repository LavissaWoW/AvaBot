...
module.exports.run = async(bot, message, args) => {
    ...
    bot.con.query(bot.queries.get_level,[mention.id,message.guild.id],function(err,profil){
        bot.con.query(bot.queries.get_global_level,[mention.id],function(err,custom){
            if(!profil || profil.length === 0) return bot.erreur("Profil not found !",message.channel.id)
            if(!custom || custom.length === 0) return bot.erreur("Profil not found !",message.channel.id)
            profil = profil[0]
            custom = custom[0]
            bot.con.query(bot.queries.get_global_leaderboard,[],async function(err,leaderboard){
                let custom_color = "#" + custom.color
                let custom_background = custom.background
                ...
                .setProgressBarTrack(custom.background === bot.config.images.rankcard_default_background ? "#2C2F33" : "TRANSPARENT","COLOR")
...
module.exports.help = {
    ...
    use_per_cooldown:1,
    deleted:false,
    description:"See someone's guild profil",
    permissions:{
        bot:"",
        user:"",
        role:""
