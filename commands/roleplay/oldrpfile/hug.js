module.exports.run = async(bot, message, args) => {
    let hug = bot.acts.hug[bot.random(0, bot.acts.hug.length - 1)]
    ...
    bot.con.query(bot.queries.get_profil,[message.author.id],function(err,profil){
        ...
        bot.con.query(bot.queries.get_profil,[mention.id],function(err,mention_profil){
            ...
            profil.huged++
            mention_profil.get_huged++
            bot.con.query(bot.queries.update_huged,[profil.huged,profil.user_id])
            bot.con.query(bot.queries.update_get_huged,[mention_profil.get_huged,mention_profil.user_id])
            message.channel.send({embed:{
                color:bot.config.colors.main,
                author:{
                    name:message.author.username + " is hugging " + (mention.id === message.author.id ? "!" : mention.username + " !"),
                ...
                footer:{
                    text:mention.username + " get huged " + mention_profil.get_huged + " times, and " + message.author.username + " huged others " + profil.huged + " times."
                }
            }})
